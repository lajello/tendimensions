...
class ExtractEmbeddingSimilarities(BaseEstimator, TransformerMixin):
    def __init__(
        ...
        self.ex = ExtractWordEmbeddings(
            emb_type=emb_type, emb_dir=emb_dir, method=method
        )

        self.dim_words = {}
        self.ground_embedding = {}
        with open(
            "/home/minje/Projects/nlpfeatures/lexicons/10_dimensions_seed_words.txt"
        ) as f:
            for line in f:
        ...
        for dim, words in self.dim_words.items():
            ...
            self.ground_embedding[dim] = (
                normalize(
                    self.ex.obtain_vectors_from_sentence(words, include_unk=False),
                    norm="l2",
                    axis=1,
                )
                .mean(0)
    ...
    def get_feature_names(self):
        return ["emb:dim-%d" % i for i in range(len(self.ground_embedding))] + [
            "emb:similarity"
        ]
...
class ExtractWordEmbeddings:
    ...
    def transform(self, X):
        ...
        if type(X[0][0]) == str:
            for sentence in X:
                arr = self.obtain_vectors_from_sentence(
                    sentence
    ...
    def get_feature_names(self):
        return [
            "wv-%s-%s:%d" % (self.emb_type, self.method, i)
            for i in range(len(self.UNK))
        ]
