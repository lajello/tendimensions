...
for dim in dimensions:
    ...
    end_time = time.time()
    print(
        f"total time = {end_time-start_time} ({(end_time-start_time)/len(df)} per entry)"
    )
...
for dim in dimensions:
    ...
    df[f"{dim}_binary_quantile_{quantile}"] = df[f"{dim}_max"].apply(
        lambda x: 0 if x < quantile_thresh else 1
    )

# binarization by fixed thresholding on the maximum value
threshold = 0.80
for dim in dimensions:
    df[f"{dim}_binary_threshold_{threshold}"] = df[f"{dim}_max"].apply(
